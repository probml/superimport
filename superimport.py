...
def get_packages_from_txt(file,dim="="):
  ...
  if dim:
      packages={c.split(dim)[0]:c.split(dim)[1] for c in packages_string.split("\n") if c}
  else:
...
def get_imports(file_string=None,patterns=[r'^import (.+)$',r'^from ((?!\.+).*?) import (?:.*)$']):
    matches=[]
    for p in patterns:
...
def import_module(module_name,verbose=False):
    try:
          ...
          if verbose:
            sys.stderr.write("ERROR: missing python module: " + module_name + "\nWill try to install automatcially\n")
          raise e
...
mapping['tensorflow-probability']="tensorflow_probability"
mapping['tensorflow-datasets']='tensorflow_datasets'
mapping['absl-py']='absl_py'
mapping['absl']='absl'
mapping['pyro-ppl']='pyro'
mapping['umap-learn']='umap_learn'
mapping['umap-learn']='umap'
mapping['dask']="dask_ml"
gnippam={v:k for k,v in mapping.items()}
